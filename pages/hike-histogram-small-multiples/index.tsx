...
const enrichedData = data.map((hike: { distance: number; ascent: number; time: number; date: string; title: string }) => ({
  ...hike,
...
const Histogram = ({ title, bucketBy, bucketByLabel }: { title: string; bucketBy: string; bucketByLabel: string }) => {
  ...
  useEffect(() => {
    ...
    const plot = Plot.plot({
      ...
      width: 1200,
      height: 200,
      facet: { data: enrichedData, x: "year" },
      x: {},
...
const Waffle = () => {
  ...
  useEffect(() => {
    ...
    const plot = Plot.plot({
      ...
      height: 450,
      marginBottom: 50,
      x: { label: null, tickFormat: " ", tickSpacing: Infinity, type: "band" },
      y: { grid: true },
      color: { scheme: "Viridis", legend: true, label: "Distance (miles)", type: "sqrt" },
      marks: [
        Plot.ruleY([0]),
        Plot.waffleY(enrichedData, {
          ...Plot.binX({ y: "count", fill: "z", sort: sortBy }, { x: "date", z: "distanceInMiles" }),
          tip: {
            format: {
              y: false,
              fill: false,
              Trail: true,
              Distance: (d: number) => `${d.toFixed(1)} miles`,
              Elevation: (d: number) => `${d.toFixed(0)} ft`,
              Time: (d: number) => `${d.toFixed(1)} hrs`,
            },
            channels: { Trail: "title", Distance: "distanceInMiles", Elevation: "ascent", Time: "timeInHours" },
          },
        }),
        Plot.axisY({ anchor: "left", label: "Count" }),
        Plot.axisX({ anchor: "bottom", label: "Date", tickFormat: "%Y", labelOffset: 40 }),
      ],
    });
    containerRef.current?.append(plot);
    return () => plot.remove();
  ...
  return (
    <div>
      <select value={sortBy} onChange={(e) => setSortBy(e.target.value as "x" | "z")} className="mb-4 border border-gray-300 rounded-md py-1 px-2">
...
const ScatterPlot = () => {
  ...
  useEffect(() => {
    const plot = Plot.plot({
      ...
      x: { type: "sqrt" },
      color: { scheme: "Viridis", legend: true, label: "Time (hours)" },
      marks: [
        Plot.circle(enrichedData, {
          ...
          channels: { Trail: "title", Time: "timeInHours", Distance: "distanceInMiles", Elevation: "ascent" },
        }),
...
const ScatterPlotSingle = () => {
  ...
  useEffect(() => {
    const plot = Plot.plot({
      ...
      color: { scheme: "Viridis", legend: true, label: "Ascent (feet)" },
      marks: [
        Plot.circle(enrichedData, {
          x: "date",
          y: "distanceInMiles",
  ...
  useEffect(() => {
    const plot = Plot.plot({
      ...
      marks: [
        ...
        Plot.areaY(
          enrichedData,
          Plot.windowY({ k: Infinity, reduce: "sum" }, {
            x: "date",
            y: "distanceInMiles",
            curve: "basis",
            tip: {
              format: {
                x: (d: Date) => new Date(d).toLocaleDateString(),
                y: (d: number) => `${d.toFixed(1)} cumulative miles`,
              },
            },
          })
        ),
    ...
    <>
      <div className="min-h-[800px] w-full">
        ...
        <div>
          ...
          <p className="mb-4">
            <Link className="underline text-blue-600 hover:text-blue-800" href="https://observablehq.com/plot/features/facets">
              Facets
            </Link>{" "}
            producing{" "}
            <Link className="underline text-blue-600 hover:text-blue-800" href="https://en.wikipedia.org/wiki/Small_multiple">
              small multiple charts
            </Link>{" "}
            were a fun one to start with. Faceting by year probably doesn&apos;t make the most sense, but I didn&apos;t have any other categories in this data.
          </p>
          <Histogram title="Hikes by Distance" bucketBy="distanceInMiles" bucketByLabel="Distance (miles)" />
          <Histogram title="Hikes by Ascent" bucketBy="ascent" bucketByLabel="Ascent (feet)" />
          <Histogram title="Hikes by Time" bucketBy="timeInHours" bucketByLabel="Time (hours)" />
          <h1 className="text-2xl font-bold text-gray-900 mb-4 mt-10">Faceted Scatterplot</h1>
          <p className="mb-4">
          ...
          <ScatterPlot />
          <h1 className="text-2xl font-bold text-gray-900 mb-4 mt-10">Single Scatterplot</h1>
          <p className="mb-4">But it might also just be better as a single plot:</p>
          <ScatterPlotSingle />
          <h1 className="text-xl font-bold text-gray-900 mb-4 mt-10">Waffle Plot</h1>
          <p className="mb-4">How about a waffle plot?</p>
          <Waffle />
          <h1 className="text-2xl font-bold text-gray-900 mb-4 mt-10">Cumulative Total</h1>
          <p className="mb-4">
            Cumulative total of hiking miles using{" "}
            <Link className="underline text-blue-600 hover:text-blue-800" href="https://observablehq.com/plot/transforms/window#window-transform">
              window transforms
            </Link>
            :
          </p>
      ...
      <h3 className="text-2xl font-semibold mt-5 mb-2 text-gray-800">Things I learned:</h3>
